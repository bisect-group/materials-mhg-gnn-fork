...
class GRUDecoder(DecoderBase):
    ...
    def init_hidden(self):
        self.hidden_dict['h'] = torch.zeros(
            (self.num_layers, self.batch_size, self.hidden_dim),
            requires_grad=False
        )
        if self.use_gpu:
...
class LSTMDecoder(DecoderBase):
    ...
    def init_hidden(self):
        ...
        self.hidden_dict['c'] = torch.zeros(
            (self.num_layers, self.batch_size, self.hidden_dim),
            requires_grad=False
        )
        if self.use_gpu:
    ...
    def forward_one_step(self, tgt_emb_in):
        ...
        Tensor, shape (batch_size, hidden_dim)
        '''
        tgt_hidden_out, self.hidden_dict['h'], self.hidden_dict['c'] = self.model(
            tgt_emb_in.view(self.batch_size, 1, -1),
