...
class GRUEncoder(EncoderBase):
    ...
    def init_hidden(self):
        self.h0 = torch.zeros(
            ((self.bidirectional + 1) * self.num_layers, self.batch_size, self.hidden_dim),
            requires_grad=False
        )
        if self.use_gpu:
    ...
    def forward(self, in_seq_emb):
        ...
        hidden_seq_emb, self.h0 = self.model(in_seq_emb, self.h0)
        hidden_seq_emb = hidden_seq_emb.view(
            self.batch_size,
            max_len,
...
class LSTMEncoder(EncoderBase):
    ...
    def forward(self, in_seq_emb):
        ...
        '''
        max_len = in_seq_emb.size(1)
        hidden_seq_emb, (self.h0, self.c0) = self.model(in_seq_emb, (self.h0, self.c0))
        hidden_seq_emb = hidden_seq_emb.view(
            self.batch_size,
            max_len,
            1 + self.bidirectional,
