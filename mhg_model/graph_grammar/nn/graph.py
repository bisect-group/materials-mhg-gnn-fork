...
class MolecularProdRuleEmbedding(nn.Module):
    ...
    def __init__(
        self, prod_rule_corpus, layer2layer_activation, layer2out_activation,
        ...
        if self.use_gpu:
            self.atom_embed = torch.randn(self.prod_rule_corpus.num_edge_symbol, self.element_embed_dim, requires_grad=True).cuda()
            self.bond_embed = torch.randn(self.prod_rule_corpus.num_node_symbol, self.element_embed_dim, requires_grad=True).cuda()
            self.ext_id_embed = torch.randn(self.prod_rule_corpus.num_ext_id, self.element_embed_dim, requires_grad=True).cuda()
            ...
            self.atom_embed = torch.randn(self.prod_rule_corpus.num_edge_symbol, self.element_embed_dim, requires_grad=True)
            self.bond_embed = torch.randn(self.prod_rule_corpus.num_node_symbol, self.element_embed_dim, requires_grad=True)
            self.ext_id_embed = torch.randn(self.prod_rule_corpus.num_ext_id, self.element_embed_dim, requires_grad=True)
            for _ in range(num_layers):
                ...
                if int(prod_rule_idx_seq[each_batch_idx, each_idx]) == len(self.prod_rule_corpus.prod_rule_list):
                    ...
                    for each_node in each_prod_rule.rhs.nodes:
                        if 'ext_id' in each_prod_rule.rhs.node_attr(each_node):
                            layer_wise_embed_dict[each_node] = layer_wise_embed_dict[each_node] + self.ext_id_embed[each_prod_rule.rhs.node_attr(each_node)['ext_id']]
                    ...
                    for each_layer in range(self.num_layers):
                        ...
                        for each_edge in each_prod_rule.rhs.edges:
                            ...
                            out[each_batch_idx, each_idx, :] = out[each_batch_idx, each_idx, :] + self.layer2out_activation(self.layer2out_list[each_layer](v))
                        for each_node in each_prod_rule.rhs.nodes:
        ...
        for each_batch_idx in range(batch_size):
            for each_idx in range(length):
                if int(prod_rule_idx_seq[each_batch_idx, each_idx]) == len(self.prod_rule_corpus.prod_rule_list):
                    ...
                    if self.use_gpu:
                        layer_wise_embed_dict = {
                            each_edge: self.atom_embed(
                                Variable(torch.LongTensor([each_prod_rule.rhs.edge_attr(each_edge)['symbol_idx']]),
