...
from .utils import _easy_node_match, _edge_match, common_node_list
...
class CliqueTreeCorpus(object):
    ...
    def add_subhg(self, subhg):
        if len(self.subhg_list) == 0:
            ...
            match = False
            subhg_bond_symbol_counter = Counter([subhg.node_attr(each_node)['symbol'] for each_node in subhg.nodes])
            subhg_atom_symbol_counter = Counter([subhg.edge_attr(each_edge).get('symbol', None) for each_edge in subhg.edges])
            for each_idx, each_subhg in enumerate(self.subhg_list):
                each_bond_symbol_counter = Counter([each_subhg.node_attr(each_node)['symbol'] for each_node in each_subhg.nodes])
                each_atom_symbol_counter = Counter([each_subhg.edge_attr(each_edge).get('symbol', None) for each_edge in each_subhg.edges])
                if not match and (
                    subhg.num_nodes == each_subhg.num_nodes
                    ...
                    try:
                        ...
                        for each_node in each_subhg.nodes:
                            subhg.node_attr(isomap[each_node])['order4hrg'] = each_subhg.node_attr(each_node)['order4hrg']
                            if 'ext_id' in each_subhg.node_attr(each_node):
                                subhg.node_attr(isomap[each_node])['ext_id'] = each_subhg.node_attr(each_node)['ext_id']
