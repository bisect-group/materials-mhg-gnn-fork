...
from .symbols import NTSymbol
from .base import GraphGrammarBase
from .corpus import CliqueTreeCorpus
from .utils import _node_match_prod_rule, _edge_match, masked_softmax, common_node_list

from ..hypergraph import Hypergraph
from ..algo.tree_decomposition import tree_decomposition_with_hrg, molecular_tree_decomposition
...
class ProductionRule(object):
    ...
    def is_same(self, prod_rule, ignore_order=False):
        ...
        subhg_bond_symbol_counter = Counter([prod_rule.rhs.node_attr(each_node)['symbol'] for each_node in prod_rule.rhs.nodes])
        each_bond_symbol_counter = Counter([self.rhs.node_attr(each_node)['symbol'] for each_node in self.rhs.nodes])
        if subhg_bond_symbol_counter != each_bond_symbol_counter:
            return False, {}

        subhg_atom_symbol_counter = Counter([prod_rule.rhs.edge_attr(each_edge)['symbol'] for each_edge in prod_rule.rhs.edges])
        each_atom_symbol_counter = Counter([self.rhs.edge_attr(each_edge)['symbol'] for each_edge in self.rhs.edges])
    ...
    def applied_to(
        ...
        if self.is_start_rule:
            ...
            for num_idx, each_node in enumerate(self.rhs.nodes):
                hg.add_node(f"bond_{num_idx}", attr_dict=self.rhs.node_attr(each_node))
            ...
            for each_edge in self.rhs.edges:
                ...
                edge_id = hg.add_edge(node_list, attr_dict=self.rhs.edge_attr(each_edge))
...
class HyperedgeReplacementGrammar(GraphGrammarBase):
    ...
    def sample(self, z, deterministic=False):
        ...
        stack = []
        z = z[:, :-1]
        init_prod_rule = self.prod_rule_corpus.sample(
            z[0], NTSymbol(degree=0,
            is_aromatic=False,
            bond_symbol_list=[]),
